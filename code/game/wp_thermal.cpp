...
//---------------------------------------------------------
{
	...
	if ( ent->health > 0 )
	{
		...
		{//NPC or misc_weapon_shooter
			...
			target[0] += Q_flrand( -5, 5 )+(Q_crandom()*(6-ent->NPC->currentAim)*2);
			target[1] += Q_flrand( -5, 5 )+(Q_crandom()*(6-ent->NPC->currentAim)*2);
			target[2] += Q_flrand( -5, 5 )+(Q_crandom()*(6-ent->NPC->currentAim)*2);
