...
//	}
	...
	for ( int i = 0; i < count; i++ )
	{
		...
		vel = BOWCASTER_VELOCITY * ( Q_crandom() * BOWCASTER_VEL_RANGE + 1.0f );
		...
		{//force sight 2+ gives perfect aim
			...
			angs[PITCH] += Q_crandom() * BOWCASTER_ALT_SPREAD * 0.2f;
			angs[YAW]	+= ((i+0.5f) * BOWCASTER_ALT_SPREAD - count * 0.5f * BOWCASTER_ALT_SPREAD );
			if ( ent->NPC )
			{
				angs[PITCH] += ( Q_crandom() * (BLASTER_NPC_SPREAD+(6-ent->NPC->currentAim)*0.25f) );
				angs[YAW]	+= ( Q_crandom() * (BLASTER_NPC_SPREAD+(6-ent->NPC->currentAim)*0.25f) );
			}
		}
