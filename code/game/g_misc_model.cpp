	...
	if ( it_ent && gun )
	{
		...
		for ( int t = 0; t < 3; t++ )
		{
			if ( rotate )
			{
				if ( t == YAW )
				{
					it_ent->s.angles[t] = AngleNormalize180( it_ent->s.angles[t] + 180 + Q_crandom() * 14 );
				}
				else
				{
					it_ent->s.angles[t] = AngleNormalize180( it_ent->s.angles[t] + Q_crandom() * 4 );
				}
			}
			else
			{
				if ( t == YAW )
				{
					it_ent->s.angles[t] = AngleNormalize180( it_ent->s.angles[t] + 90 + Q_crandom() * 4 );
				}
			}
		}
...
void SP_misc_model_gun_rack( gentity_t *ent )
{
	...
	if ( ct )
	{
		for ( int i = 0; i < ct; i++ )
		{
			GunRackAddItem( itemList[i], ent->s.origin, ent->s.angles, Q_crandom() * 2, ( i - 1 ) * 9 + Q_crandom() * 2, ofz[i] );
		}
	}
...
void spawn_rack_goods( gentity_t *ent )
{
	...
	if ( ct )
	{
		for ( int i = 0; i < ct; i++ )
		{
			GunRackAddItem( itemList[i], ent->s.origin, ent->s.angles, Q_crandom() * 0.5f, (i-1)* 8, 7.0f );
		}
	}
	...
	if ( ent->spawnflags & RACK_WEAPONS )
	{
		...
		{
			// if weapon is checked...and so are one or more ammo types, then pick a Q_random weapon to display..always give weaker weapons first
			if ( blaster )
			{
				it = blaster;
				v_off = 25.5f;
		...
		if ( it )
		{
			...
			GunRackAddItem( it, ent->s.origin, ent->s.angles, Q_crandom() * 2, ( Q_random() * 6 + 4 ) * pos, v_off );
		}
	}
	...
	if (( ent->spawnflags & RACK_HEALTH ) && health )
	{
		...
		GunRackAddItem( health, ent->s.origin, ent->s.angles, Q_crandom() * 0.5f, ( Q_random() * 4 + 4 ) * pos, 24 );
	}
...
void misc_model_cargo_die( gentity_t *self, gentity_t *inflictor, gentity_t *attacker, int damage, int mod, int dFlags, int hitLoc )
{
	...
	if (( flags & DROP_MEDPACK ))
	{
		...
		if ( health )
		{
			temp[0] = org[0] + Q_crandom() * 8 + 16;
			temp[1] = org[1] + Q_crandom() * 8 + 16;
